...
function SubmitToOlympiad({ olympiadId, wpm, readPercent, readElapsed, quizScore, submitted, onSubmitted }) {
    ...
    if (status === 'sending') return <p className="text-white/40 text-xs text-center animate-pulse">Natija olimpiadaga yuborilmoqda...</p>;
    if (status === 'done') return <p className="text-emerald-400 text-xs text-center">✅ Natija olimpiadaga yuborildi!</p>;
    return null;
}

function RecordingModal({ ertak, onClose, olympiadId = null }) {
        ...
        <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] to-[#16213e] relative overflow-hidden">
            ...
            <header className="relative z-10 border-b border-white/10 backdrop-blur-md bg-white/5">
                <div className="max-w-6xl mx-auto px-4 py-5 flex items-center justify-between">
                    ...
                    <Link to={fromReading ? `/olympiad/${olympiadId}/reading` : `/olympiad/${olympiadId}/content`} className="flex items-center gap-2 text-white/60 hover:text-white transition-colors text-sm">
            ...
            <AnimatePresence>
                {activeErtak && (
                    <RecordingModal ertak={activeErtak} onClose={() => setActiveErtak(null)} olympiadId={olympiadId} />
                )}
            </AnimatePresence>
        </div>
    );
