...
def upgrade():
    # Add type and grade_level columns to olympiads table
    op.add_column('olympiads', sa.Column('type', sa.Enum('test', 'reading', 'mixed', name='olympiadtype'), server_default='test', nullable=True))
    op.add_column('olympiads', sa.Column('grade_level', sa.String(20), nullable=True))

    # Create olympiad_reading_tasks table
    op.create_table(
    ...
    # Create olympiad_reading_submissions table
    op.create_table(
    ...
    # Indexes
    op.create_index('ix_reading_tasks_olympiad', 'olympiad_reading_tasks', ['olympiad_id'])
    op.create_index('ix_reading_subs_participant', 'olympiad_reading_submissions', ['participant_id'])
    op.create_index('ix_reading_subs_task', 'olympiad_reading_submissions', ['reading_task_id'])
