...
async function check() {
  ...
  try {
    ...
    console.log(`\n--- global books (total: ${booksRes.rowCount}) ---`);
    ...
    console.log(`\n--- store_books (total: ${storeBooksRes.rowCount}) ---`);
    ...
    storesRes.rows.forEach((s) => {
      console.log(`  [Store ID: ${s.id}] "${s.name}" (Owner ID: ${s.owner_id}, BookCount: ${s.book_count})`);
    });

  } catch (err) {
    console.error("Postgres connection or query failed:", err.message);
  } finally {
    await client.end();
  }
}
